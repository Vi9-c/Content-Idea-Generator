...
/**
 * Handles the click event for generating content ideas using the secure proxy.
 */
async function generateIdeas() {
  ...
  contentOutput.innerHTML = `<p class="text-center text-cyan-500 italic p-4">Initializing AI Content Matrix... Please wait. (Contacting secure proxy...)</p>`;

  // Payload only contains the data needed by the server
  const payload = {
    businessType,
    targetTopic,
    contentFormat,
  ...
  try {
    ...
    // The response structure is { content: "..." } from the proxy
    if (result && result.content) {
      const generatedText = result.content;
      contentOutput.innerHTML = formatLLMResponse(generatedText);
    } else {
      displayError(
        "ERROR: Null data received from proxy. AI response was empty."
      );
    ...
    displayError(
      `CRITICAL ERROR: Connection failure. Check that the Node.js server is running. (${error.message})`
    );
